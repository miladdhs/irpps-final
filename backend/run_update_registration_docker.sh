#!/bin/bash
# اسکریپت اجرای دستور update_events_registration داخل Docker container
# برای به‌روزرسانی registration_deadline رویدادهای موجود
...
echo -e "${BLUE}🔍 بررسی وجود دستور update_events_registration...${NC}"
if ! docker compose exec -T backend test -f /app/events/management/commands/update_events_registration.py 2>/dev/null; then
    echo -e "${YELLOW}⚠️  دستور update_events_registration در container پیدا نشد${NC}"
...
echo -e "${YELLOW}🚀 در حال اجرای دستور: python manage.py update_events_registration${NC}"
echo ""

# اجرای دستور داخل container
docker compose exec -T backend python manage.py update_events_registration
