<template>
  <div class="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100">
      ...
      <main class="mx-auto w-full max-w-[1280px] grow px-6 py-8 lg:px-10">
        ...
        <div class="mb-8 flex w-full max-w-2xl overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div class="flex flex-1 items-center px-4">
            ...
            <input v-model="searchQuery" class="w-full border-none bg-transparent py-4 text-sm focus:ring-0" :placeholder="$t('team.search')" type="text" />
          </div>
          <button v-if="searchQuery" @click="searchQuery = ''" class="bg-slate-100 px-6 font-medium text-slate-600 hover:bg-slate-200 transition-colors dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700">
    ...
    <div v-if="showModal && selectedMember" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="closeModal">
...
const copy = computed(() => (locale.value === 'fa'
  ? {
      ...
      fetchError: 'خطا در دریافت اطلاعات اعضا',
    }
  : {
      unknownName: 'Unknown',
      serverError: 'Server error',
      fetchError: 'Failed to load member information',
    }))

const getMemberName = (member: Member): string => {
  if (locale.value === 'fa') {
    return member.persian_name || member.english_name || member.display_name || copy.value.unknownName
  }
  return member.english_name || member.persian_name || member.display_name || copy.value.unknownName
}
...
async function fetchMembers() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`${copy.value.serverError} (${response.status})`)
    }
    ...
    memberError.value = copy.value.fetchError
  } finally {
    loading.value = false
  }
}
...
const filteredMembers = computed(() => {
  ...
  return members.value.filter((member) => {
    const searchableFields = [member.display_name, member.persian_name, member.english_name, member.specialty, member.city]
